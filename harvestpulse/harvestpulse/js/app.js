...
function fmtCurrency(n) {
  return 'R' + parseFloat(n).toFixed(2).replace(/\.00$/, '');
}
...
function incBid(amount) {
  myBidAmount = parseFloat((myBidAmount + amount).toFixed(2));
...
function buildDashboard() {
  ...
  const d = farmerData;
  const maxEarned = Math.max(...(d.chart.map(c => parseFloat(c.earned)) || [1]));
  return `
...
function attachProduceEvents() {
  document.querySelectorAll('.produce-item').forEach(el => {
    el.addEventListener('click', () => {
      selProduce = parseInt(el.dataset.idx);
...
async function submitListing() {
  ...
  const payload = {
    ...
    weight_kg: parseFloat(document.getElementById('fWeight').value) || 0,
    reserve: parseFloat(document.getElementById('fReserve').value),
    retail: parseFloat(document.getElementById('fRetail').value),
    minutes: selWindow,
            ...
            <tbody>
              ${rows.map(r => `
                <tr>
                  ...
                  <td><span class="${r.status === 'sold' ? 'badge-sold' : r.status === 'live' ? 'badge-live' : 'badge-exp'}">${r.status}</span></td>
                </tr>
              `).join('')}
            </tbody>
          </table>
...
document.addEventListener('DOMContentLoaded', () => {
  // Start countdowns running on page load
  startCountdowns();
});
